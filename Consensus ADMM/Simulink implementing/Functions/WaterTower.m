...
% Making the consensus problem for the water 
...
total=c.Nc*c.Nu;
%% setting fmincon to use sqp
options = optimoptions(@fmincon,'Algorithm','sqp');
%Need a few empty matrix: 
Aeq=[];
beq=[];
lb=[];
ub=[];
...
%Defining the part of the cost function which is in regard to the ADMM 
J_con_z = @(u) lambda'*(u-z)+c.rho/2*((u-z)'*(u-z));


%Writting up the cost function 

costFunctionAll=@(u) (Js(u)+J_con_z(u));

%Initial guess
x0 =[x(3:end,1);x(end-1:end,1)]; 
