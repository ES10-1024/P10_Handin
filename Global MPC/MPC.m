...
for time=1:3600/c.ts*Tsim
    ...
    %Making vector of the electricity prices
    ElPrices(:,index)=ElectrictyPrices(CurrentTime(index+1),CurrentDay);
    %Saving current electricity price:
...
xlabel('Samples [*]')
legend('Pumpe 1','Pumpe 2','Pumpe 3','Constraint 1','Constraint 2','Constraint 3')

if exportGraphs==true 
exportgraphics(f,'Results/Amount_pumped_one_day.pdf')
end 
%% Plotting electricity bill 
clf 
plot(Bill(2:end,1)) 
grid on 
xlabel('Samples [*]')
